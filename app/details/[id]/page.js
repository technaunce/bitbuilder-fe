...
import { useProductDetails } from "../../store/details";

const Details = () => {
  const [isPageLoading, setIsPageLoading] = useState(true);
  const [isPageError, setIsPageError] = useState(false);
  const [productDetailsLocal, setProductDetailsLocal] = useState({});
  const { context, getProductDetails, setDetails } = useProductDetails();
  ...
  const handleDetails = (status, response) => {
    ...
    if (status) {
      ...
      // Set all data to store and state
    } else {
      setIsPageError(true);
    }
  };
  ...
  useEffect(() => {
    getProductDetails(params.id, handleDetails);
  }, []);
  //dummy test data
  const content = [
    {
      ...
      <section className="container mx-auto mb-10">
        <div className="flex justify-between lg:flex-col lg:mt-9 ">
          <div className="w-3/5 mr-28 lg:mr-0 lg:w-full">
            <SubmitBtn view={"mob"} />
            <h6 className="text-2xl font-medium mb-5 lg:text-xl mt-[17px]">
              Description
            </h6>
            ...
            <hr className="h-[1px] w-full mt-9 mb-10 bg-grey-keyline" />
            <DetailTabs />
            <h6 className="mb-5 text-2xl font-medium lg:text-xl">
              Average price
            </h6>
            <p className="text-2xl text-grey-body leading-[34px] mb-8 lg:text-base md:mb-5">
...
const SubmitBtn = ({ view }) => {
  return (
    <button
      className={`flex-col justify-center items-center w-full p-[10px] bg-secondary transition group hover:bg-primary ${
        view === "mob" ? "hidden lg:flex" : "flex lg:hidden"
      }  md:text-base`}
    >
